...
def auth_social_media(request):
    ...
    if (user.id,network) not in pending_social_accounts:
        ...
        return HttpResponseRedirect("/powermonitorweb/manage_social_media/")
...
def manage_reports(request):
    ...
    if request.is_ajax():
        ...
        if datadict.get('identifier') == 'id_report_type_change':
            ...
            report_to_change = user_reports.get(report_id=report)
...
def generate_usage_graph(period_type, length, file_path):
        ...
        frame = dfc().collect_period(period_type=period_type,
                                     period_start=datetime.now().replace(microsecond=0) - delta,
...
def get_current_statistics():
    ...
    print 'ESKOM_COLOUR %s' % eskom_colour
    print 'ESKOM_USAGE %s' % eskom_usage
